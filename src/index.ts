/**
 * Public entry point for the `@cosyte/terminology` package.
 *
...
/**
 * The result of parsing a Terminology payload.
 *
 ...
 */
export interface ParsedTerminology {
...
/**
 * Options for {@link parseTerminology}.
 *
 * The parser is **lenient by default** (Postel's Law): vendor quirks become {@link TerminologyWarning}s
...
/**
 * Parse a Terminology payload into a {@link ParsedTerminology}.
 *
 * **Lenient by default** — real-world, vendor-quirky input parses rather than throws, emitting
 * {@link TerminologyWarning}s instead (Postel's Law). The complementary serializer (added in a later
 * phase) always emits spec-clean output. Only unrecoverable structural corruption throws (a Tier-3
 * {@link FatalCode}).
 *
 * @param raw - The raw Terminology payload (`string` for now; `Buffer`/`Uint8Array` support is added
 *   alongside the real parser).
 * @param options - Parse options; see {@link ParseTerminologyOptions}. Lenient unless `strict` is set.
 ...
 * @example
 * ```ts
 * import { WARNING_CODES } from "@cosyte/terminology";
 *
